    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: cores.background }]}>
      ...
      <View style={styles.analiseContainer}>
        ...
        {step === "preparing" ? (
          ...
          >
            <View style={[styles.bigCircle, { backgroundColor: cores.primary }]}>
              <Ionicons name="eye-outline" size={44} color={cores.surface} />
            </View>
            <Text style={[styles.prepTitle, { color: cores.primary }]}>{t('analise_preparando')}</Text>
            <Text style={[styles.prepDesc, { color: cores.text }]}>
              {t('analise_instrucoes')}
            </Text>
            <TouchableOpacity style={[styles.ctaButtonLarge, { backgroundColor: cores.primary, shadowColor: cores.shadow }]} onPress={() => setStep("choose-method")}> 
              <Text style={[styles.ctaButtonTextLarge, { color: cores.surface }]}>{t('analise_continuar')}</Text>
            </TouchableOpacity>
          </Animated.View>
        ) : step === "choose-method" ? (
          <Animated.View
          ...
          >
            ...
            <View style={styles.chooseRow}>
              <TouchableOpacity style={[styles.chooseButton, { backgroundColor: cores.primary, shadowColor: cores.shadow }]} onPress={() => setStep("camera")}> 
                <Ionicons name="camera" size={38} color={cores.surface} style={{ marginBottom: 8 }} />
                <Text style={[styles.chooseButtonText, { color: cores.surface }]}>{t('analise_tirar_foto')}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.chooseButton, { backgroundColor: cores.primary, shadowColor: cores.shadow }]} onPress={() => setStep("gallery")}> 
                <Ionicons name="images" size={38} color={cores.surface} style={{ marginBottom: 8 }} />
                <Text style={[styles.chooseButtonText, { color: cores.surface }]}>{t('analise_galeria')}</Text>
              </TouchableOpacity>
            </View>
          </Animated.View>
        ) : step === "camera" ? (
          <View style={[styles.cameraFullContainer, { paddingTop: 0 }]}> 
            {/* Botão de voltar (esconde quando resultado aparecendo) */}
            {!showResult && (
              <TouchableOpacity style={[styles.backButton, { backgroundColor: cores.surface, shadowColor: cores.shadow }]} onPress={() => { setPhotoUri(null); setStep("choose-method"); }}>
                <Ionicons name="arrow-back" size={28} color={cores.primary} />
              </TouchableOpacity>
            )}
            {hasCameraPermission === false ? (
              ...
              <View style={styles.cameraPreviewContainer}>
                ...
                <View style={{ flexDirection: 'row', gap: 12, width: '100%', justifyContent: 'center' }}>
                  <TouchableOpacity style={[styles.ctaButtonLarge, { flex: 1, backgroundColor: cores.surface, borderWidth: 1, borderColor: cores.primary, shadowColor: cores.shadow }]} onPress={() => setPhotoUri(null)}>
                    <Text style={[styles.ctaButtonTextLarge, { color: cores.primary }]}>{t('analise_tirar_outra')}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.ctaButtonLarge, { flex: 1, backgroundColor: cores.primary, shadowColor: cores.shadow }]} onPress={() => handleConfirmImage(photoUri)}>
                    <Text style={[styles.ctaButtonTextLarge, { color: cores.surface }]}>{t('analise_confirmar')}</Text>
                  </TouchableOpacity>
                </View>
              </View>
            ) : (
              <>
                  {!showResult && (
                    <CameraView
                      ...
                      style={{ position: 'absolute', top: HEADER_HEIGHT, left: 0, right: 0, bottom: NAVBAR_HEIGHT, width: '100%', height: '100%' }}
                      facing="back"
                      flash={flash}
                    />
                  )}
                {!showResult && (
                  <>
                    <TouchableOpacity
                      style={[styles.flashButton, { backgroundColor: `rgba(0,196,125,0.9)`, shadowColor: cores.shadow }]}
                      onPress={() => setFlash(flash === 'off' ? 'on' : 'off')}
                    >
                      <Ionicons name={flash === 'off' ? "flash-off" : "flash"} size={28} color={cores.surface} />
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[styles.captureButton, { backgroundColor: `rgba(0,196,125,0.95)`, shadowColor: cores.shadow }]}
                      onPress={async () => {
                    ...
                    >
                      <Ionicons name="camera" size={36} color={cores.surface} />
                    </TouchableOpacity>
                  </>
                )}
              </>
          ...
          <View style={styles.centerContent}>
            {/* Botão de voltar */}
            <TouchableOpacity style={[styles.backButton, { backgroundColor: cores.surface, shadowColor: cores.shadow }]} onPress={() => { setGalleryImage(null); setStep("choose-method"); }}>
              <Ionicons name="arrow-back" size={28} color={cores.primary} />
            </TouchableOpacity>
            {galleryImage ? (
              <View style={styles.cameraPreviewContainer}>
                ...
                <View style={{ flexDirection: 'row', gap: 12, width: '100%', justifyContent: 'center' }}>
                  <TouchableOpacity style={[styles.ctaButtonLarge, { flex: 1, backgroundColor: cores.surface, borderWidth: 1, borderColor: cores.primary, shadowColor: cores.shadow }]} onPress={() => setGalleryImage(null)}>
                    <Text style={[styles.ctaButtonTextLarge, { color: cores.primary }]}>{t('analise_escolher_outra')}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.ctaButtonLarge, { flex: 1, backgroundColor: cores.primary, shadowColor: cores.shadow }]} onPress={() => handleConfirmImage(galleryImage)}>
                    <Text style={[styles.ctaButtonTextLarge, { color: cores.surface }]}>{t('analise_confirmar')}</Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <>
                <View style={styles.cameraBoxEnhanced}>
                  <View style={styles.cameraCircleDashedLarge}>
                    <Ionicons name="images" size={64} color={cores.primary} style={{ opacity: 0.7 }} />
                  </View>
                </View>
                <TouchableOpacity
                  style={[styles.ctaButtonLarge, { backgroundColor: cores.primary, shadowColor: cores.shadow }]}
                  onPress={async () => {
                    ...
                    if (!result.canceled && result.assets && result.assets[0]?.uri) {
                      ...
                      // Aceita qualquer imagem, pois o ImagePicker já filtra
                      setGalleryImage(uri);
                    } else if (result.canceled) {
                      // nada
                    } else {
                      alert('Selecione um arquivo .png, .jpg ou .jpeg');
                    }
                  }}
                >
                  <Text style={[styles.ctaButtonTextLarge, { color: cores.surface }]}>{t('analise_escolher_galeria')}</Text>
                </TouchableOpacity>
              </>
            )}
          </View>
        ...
        {processing && (
          <View style={[styles.centerContent, { backgroundColor: cores.background, position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 99, justifyContent: 'center' }]}> 
            <Animated.View style={{
              width: 160,
              height: 160,
              borderRadius: 80,
              ...
              <Ionicons name="cloud-upload-outline" size={54} color={cores.primary} style={{ opacity: 0.7 }} />
            </Animated.View>
            <Text style={[styles.prepTitle, { marginTop: 32, color: cores.primary }]}>{t('analise_processando')}...</Text>
          </View>
        )}
        {/* Resultado final - overlay centralizado */}
        {showResult && (
          <Animated.View style={{ position: 'absolute', top: HEADER_HEIGHT + 8, left: 12, right: 12, bottom: NAVBAR_HEIGHT + 8, zIndex: 1500, justifyContent: 'center', alignItems: 'center', transform: [{ translateY: showResultAnim }] }}>
            <View style={{ width: '100%', maxWidth: 520 }}>
              <View style={[styles.resultCard, { backgroundColor: cores.surface, shadowColor: cores.shadow }]}>
                <View style={{ backgroundColor: cores.primary, borderRadius: 44, width: 88, height: 88, alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                  <Ionicons name="checkmark-circle" size={54} color={cores.surface} />
                </View>
                <Text style={{ fontSize: 20 * fontScale, fontWeight: '700', color: cores.primary, marginBottom: 8, textAlign: 'center', flexWrap: 'wrap' }}>{t('analise_nenhum_sinal')}</Text>
                <Text style={{ fontSize: 15 * fontScale, color: cores.text, marginBottom: 18, textAlign: 'center', flexWrap: 'wrap' }}>{t('analise_nenhum_sinal_desc')}</Text>
                <View style={{ backgroundColor: cores.primary, borderRadius: 12, paddingVertical: 10, paddingHorizontal: 18, alignSelf: 'stretch' }}>
                  <Text style={{ color: cores.surface, fontWeight: '700', fontSize: 15 * fontScale, textAlign: 'center', flexWrap: 'wrap' }}>{t('analise_recomendacao')}</Text>
                </View>
              </View>
              ...
              <View style={[styles.sectionCard, { backgroundColor: cores.surface, shadowColor: cores.shadow }]}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                  <Ionicons name="calendar-outline" size={22 * fontScale} color={cores.primary} style={{ marginRight: 8 }} />
                  <Text style={{ fontSize: 14 * fontScale, color: cores.text, flexWrap: 'wrap' }}>{t('analise_agendar_proxima')} <Text style={{ color: cores.primary, fontWeight: '700' }}>→</Text></Text>
                </View>
                <Text style={{ fontSize: 13 * fontScale, color: '#3D6656', marginLeft: 30, flexWrap: 'wrap' }}>{t('analise_agendar_3_meses')}</Text>
              </View>
                ...
                <Animated.View style={[styles.detailsContent, { height: detailsAnim.interpolate({ inputRange: [0,1], outputRange: [0,120] }) }]}>
                  <View style={styles.detailRow}>
                    <View style={styles.detailCol}>
                      ...
                      <Text style={[styles.detailValue, { color: cores.primary }]}>{demoDate}</Text>
                    </View>
                    <View style={styles.detailCol}>
                      <Text style={[styles.detailLabel, { color: cores.text }]}>{t('analise_hora')}</Text>
                      <Text style={[styles.detailValue, { color: cores.primary }]}>{demoTime}</Text>
                    </View>
                  </View>
                  <View style={styles.detailRow}>
                    <View style={styles.detailCol}>
                      <Text style={[styles.detailLabel, { color: cores.text }]}>{t('analise_confianca')}</Text>
                      <Text style={[styles.detailValue, { color: cores.primary }]}>87%</Text>
                    </View>
                    <View style={styles.detailCol}>
                      <Text style={[styles.detailLabel, { color: cores.text }]}>{t('analise_olho_analisado')}</Text>
                      <Text style={[styles.detailValue, { color: cores.primary }]}>{t('analise_ambos')}</Text>
                    </View>
                  </View>
                </Animated.View>
              </View>
              ...
              <View style={styles.actionRow}>
                ...
                <TouchableOpacity style={[styles.actionBtn, { backgroundColor: cores.surface, shadowColor: cores.shadow }]} onPress={() => { setConfirmedImage(null); setShowResult(false); setPhotoUri(null); setGalleryImage(null); setStep("choose-method"); }}>
