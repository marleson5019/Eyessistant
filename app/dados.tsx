  ...
  <Text style={[styles.dashTitle, { fontSize: 18 * fontScale, color: cores.primary }]}>{t('dados_titulo')}</Text>
        ...
        <View style={styles.graphCardWrap}>
          <View style={[styles.graphCard, { minHeight: 180, paddingTop: 48, paddingBottom: 24, justifyContent: 'flex-start', backgroundColor: cores.surface, borderWidth: 0, shadowColor: cores.shadow, shadowOpacity: 0.18, shadowRadius: 18, elevation: 10 }]}> 
            <Text style={[styles.graphTitle, { marginBottom: 28, fontSize: 15 * fontScale }]}>Análises por mês</Text>
            <View style={styles.barGraphRowMultiCol}>
              {analisesPorMes.map((val, i) => (
                <View key={i} style={styles.barColMulti}>
                  ...
                  <Text style={[styles.barLabel, { fontSize: 12 * fontScale }]}>{meses[i]}</Text>
        ...
        <View style={styles.graphCardWrap}>
          <View style={[styles.graphCard, { backgroundColor: cores.surface, borderWidth: 0, shadowColor: '#FFD600', shadowOpacity: 0.13, shadowRadius: 18, elevation: 10 }]}> 
            <Text style={[styles.graphTitle, { fontSize: 15 * fontScale }]}>Proporção de diagnósticos</Text>
            <View style={{ marginTop: 18, marginBottom: 8, width: '100%' }}>
              {[{
                label: 'Sinais de Catarata', color: '#FFD600', value: positivos
              }, {
                label: 'Normais', color: '#00A86B', value: negativos
              }].map((item, idx) => (
                <View key={item.label} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 14 }}>
                  ...
                  <Text style={{ marginLeft: 12, fontSize: 15, color: '#3D6656', fontWeight: '600' }}>{item.label}</Text>
                </View>
              ))}
            </View>
          </View>
          ...
          {[{
            ...
            >
              ...
              >
                ...
                {/* Gradiente/Brilho fake */}
                <View style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 18, borderTopLeftRadius: 22, borderTopRightRadius: 22, backgroundColor: '#fff', opacity: 0.13 }} />
              </TouchableOpacity>
            </Animated.View>
          ))}
        </View>
